...
MAX_CAPITAL = 500000
NIFTY_200_CHECK = True
BACK_TEST_DAYS = 365
EMA_HISTORY_DAYS = 500
TOP_N_SELECT = 30
TOP_N_INVEST = 15
REBALANCE_COUNTER = 5
SIP_AMOUNT = 0
...
class MomentumFinal:
    ...
    def __init__(self, execution_date_obj, userid, debug_mode):
        ...
        logger.info(f"Initializing Momentum Trading Strategy for User ID: {userid}")
        ...
        self.rebalance_counter = 1 if self.rebalance_today else current_portfolio_data_from_excel.get("Rebalance_Counter", 1)
        ...
        sendeqtAlert(f"{self.client_name} - Holdings: {len(self.portfolio)} | Cash: {self.cash_remaining:,.0f} | Rebal: {self.rebalance_counter}")
    ...
    def get_stock_history(self):
        ...
        if not os.path.exists(stock_history_file):
            ...
            nifty_200_file = os.path.join(os.path.dirname(Path(__file__)), "stocks_list.csv")
            ...
            if LIQUID_ETF not in stock_list:
                stock_list.append(LIQUID_ETF)
            total_stock_history = {}
    ...
    def place_order(
        ...
        logger.info(f"DEBUG MODE FLAG: {'ENABLED' if debug_mode else 'DISABLED'}")
        if exchange == "NSE":
        ...
        if tran_type == "BUY":
            ...
            limitprice = round(limitprice + limitprice * 0.02) if not self.debug_mode else round(limitprice)
        elif tran_type == "SELL":
        ...
        for retry_count in range(3):
            try:
                ...
                # PHASE 1 - CHANGE 5: Timeout to prevent infinite loop on stuck orders
                max_wait_seconds = 60
                ...
                average_price = 0
                status = "REJECTED"
                # PHASE 1 - CHANGE 3: str() for reliable string comparison
        ...
        # PHASE 1 - CHANGE 1: Return REJECTED instead of raising exception.
        # All 3 callers already handle REJECTED gracefully - stock is simply skipped.
    ...
    def buy_stocks_or_not(self):
        if NIFTY_200_CHECK:
            ...
            sendeqtAlert(f"{self.client_name} - Market: {market_condition} (MC150: {nifty_200_closing_price:.0f} vs EMA: {nifty_200_ema:.0f})")
    ...
    def get_ema_for_date(self, stock_name, target_date, period=100):
        ...
        if isinstance(series, pd.Series):
            ...
            ema_pandas = filtered_series.ewm(span=period, adjust=False).mean()
            result = round(ema_pandas.iloc[-1], 2)
            self._ema_cache[cache_key] = result
            return result
    ...
    def populate_stock_top_select_and_invest_data(self):
        ...
        valid_stocks = today_prices[today_prices < 25000].index
    ...
    def sell_stocks_and_reclaim_capital(self, stocks_to_sell):
        ...
        logger.info(f"Number of stocks to sell: {len(stocks_to_sell)}")
        logger.info("-" * 60)
        ...
        for stock in stocks_to_sell:
            ...
            order_id, shares, average_price, status, status_message = self.place_order(
                "NSE", "SELL", stock, "MKT", shares, sell_price, self.debug_mode
            )
            if status == 'COMPLETE':
        ...
        if stocks_deleted:
            ...
            sendeqtAlert(
                f"{self.client_name} - Sold {len(stocks_deleted)}: "
    ...
    def get_stocks_to_buy(self, remaining_slots):
        ...
        sendeqtAlert(f"{self.client_name} - Cash: {self.cash_remaining:,.0f} | Per stock: {investment_per_stock:,.0f} | Slots: {remaining_slots}")
        ...
        logger.info("-" * 75)
        logger.info(f"Selected {len(stocks_to_buy)} stocks for purchase")
    ...
    def buy_stocks_and_spend_capital(self, stocks_to_buy):
        ...
        remaining_slots = TOP_N_INVEST - sum(1 for s in self.portfolio if s != LIQUID_ETF)
        investment_per_stock = self.cash_remaining / remaining_slots
        logger.info(f"Investment per stock: {investment_per_stock:,.2f}")
        ...
        for stock in stocks_to_buy.keys():
            ...
            if shares > 0:
                order_id, shares, average_price, status, status_message = self.place_order(
                    "NSE", "BUY", stock, "MKT", shares, buy_price, self.debug_mode
                )
                if status == 'COMPLETE':
                    ...
                    sendeqtAlert(f"{self.client_name} - BUY REJECTED: {stock} - {status_message}")
            else:
        ...
        # Alert with bought stock names
        if stocks_added:
            bought_names = [s.replace('NSE:', '') for s in stocks_added.keys()]
            sendeqtAlert(
                f"{self.client_name} - Bought {len(stocks_added)}: "
    ...
    def get_portfolio_stocks_fell_below_ema(self):
        ...
        logger.info("-" * 75)
        logger.info(f"Stocks to sell due to EMA breach: {len(stocks_to_sell)}")

        if stocks_to_sell:
            # Alert only when there are actual EMA breaches with stock names
            ema_stocks_str = ", ".join([s.replace("NSE:", "") for s in stocks_to_sell.keys()])
            sendeqtAlert(f"{self.client_name} - EMA BREACH ({len(stocks_to_sell)}): {ema_stocks_str}")
            logger.info("EMA BREACH DETECTED - PREPARING TO SELL")
        else:
            sendeqtAlert(f"{self.client_name} - EMA check done. No breach. {len(self.portfolio)} holdings safe")
            logger.info("All portfolio stocks above EMA - no immediate sells needed")
    ...
    def sweep_cash_to_liquid_etf(self):
        """
        Buy LIQUIDCASE ETF with all available cash to minimize idle cash.
        Called on rebalance day when market is BEARISH.
        """
        logger.info("SWEEP CASH INTO LIQUID ETF")
        logger.info("-" * 60)
        current_price_data = self.get_last_price([LIQUID_ETF], debug_mode=self.debug_mode)
        buy_price = current_price_data[LIQUID_ETF]["last_price"]
        shares = int(self.cash_remaining / buy_price)

        if shares <= 0:
            logger.info(f"Cash {self.cash_remaining:,.2f} insufficient to buy even 1 unit of {LIQUID_ETF} at {buy_price}")
            return

        order_id, shares, average_price, status, status_message = self.place_order(
            "NSE", "BUY", LIQUID_ETF, "MKT", shares, buy_price, self.debug_mode
        )
        if status == 'COMPLETE':
            spent = round(shares * average_price, 2)
            self.cash_remaining -= spent
            if LIQUID_ETF in self.portfolio:
                # Accumulate into existing position — update average buy price
                existing = self.portfolio[LIQUID_ETF]
                total_shares = existing["No_Of_Shares"] + shares
                total_buy_amount = round(existing["Buy_Amount"] + spent, 2)
                self.portfolio[LIQUID_ETF] = {
                    "No_Of_Shares": total_shares,
                    "Current_Price": round(average_price, 2),
                    "Current_Amount": round(total_shares * average_price, 2),
                    "Buy_Price": round(total_buy_amount / total_shares, 2),
                    "Buy_Amount": total_buy_amount,
                    "Entry_Date": existing["Entry_Date"],
                    "Holding_Days": existing["Holding_Days"],
                }
                logger.info(f"Added {shares} units to existing {LIQUID_ETF} | Total: {total_shares} units | Avg Buy: {self.portfolio[LIQUID_ETF]['Buy_Price']} | Cash left: {self.cash_remaining:,.2f}")
                sendeqtAlert(f"{self.client_name} - LIQUIDCASE ADD: {shares} units | Total: {total_shares} | Avg: {self.portfolio[LIQUID_ETF]['Buy_Price']} | Cash left: {self.cash_remaining:,.0f}")
            else:
                self.portfolio[LIQUID_ETF] = {
                    "No_Of_Shares": shares,
                    "Current_Price": round(average_price, 2),
                ...
                logger.info(f"Bought {shares} units of {LIQUID_ETF} at {average_price} | Spent: {spent:,.2f} | Cash left: {self.cash_remaining:,.2f}")
                sendeqtAlert(f"{self.client_name} - LIQUIDCASE BUY: {shares} units at {average_price} | Cash left: {self.cash_remaining:,.0f}")
        else:
            logger.error(f"LIQUIDCASE BUY order rejected: {status_message}")
            sendeqtAlert(f"{self.client_name} - LIQUIDCASE BUY REJECTED: {status_message}")

    def exit_liquid_etf(self):
        """
        Sell all LIQUIDCASE ETF units to free cash for momentum buys.
        Called on rebalance day when market turns BULLISH.

        Returns:
            OrderedDict: Exit details for write_exit_stocks_to_excel()
        """
        logger.info("EXIT LIQUID ETF - MARKET TURNED BULLISH")
        logger.info("-" * 60)
        stocks_deleted = OrderedDict()

        if LIQUID_ETF not in self.portfolio:
            logger.info(f"{LIQUID_ETF} not in portfolio - nothing to exit")
            return stocks_deleted

        shares = int(self.portfolio[LIQUID_ETF]["No_Of_Shares"])
        current_price_data = self.get_last_price([LIQUID_ETF], debug_mode=self.debug_mode)
        sell_price = current_price_data[LIQUID_ETF]["last_price"]

        order_id, shares, average_price, status, status_message = self.place_order(
            "NSE", "SELL", LIQUID_ETF, "MKT", shares, sell_price, self.debug_mode
        )
        if status == 'COMPLETE':
            proceeds = round(shares * average_price, 2)
            self.cash_remaining += proceeds
            buy_amount = round(self.portfolio[LIQUID_ETF]["Buy_Amount"], 2)
            profit_loss = round(proceeds - buy_amount, 2)
            entry_date_obj = datetime.strptime(self.portfolio[LIQUID_ETF]["Entry_Date"], "%d-%m-%Y")

            stocks_deleted[LIQUID_ETF] = {
                "Entry_Date": self.portfolio[LIQUID_ETF]["Entry_Date"],
                "Exit_Date": self.execution_date_str,
                "Holding_Days": (self.execution_date_obj - entry_date_obj).days + 1,
                "No_Of_Shares": shares,
                "Buy_Price": self.portfolio[LIQUID_ETF]["Buy_Price"],
                "Buy_Amount": buy_amount,
                "Sell_Price": round(average_price, 2),
                "Sell_Amount": proceeds,
                "100_Days_EMA": 0,
                "Profit_Loss": profit_loss,
                "Percentage": round((profit_loss / buy_amount) * 100, 2) if buy_amount else 0,
            }
            del self.portfolio[LIQUID_ETF]
            logger.info(f"Sold {shares} units of {LIQUID_ETF} at {average_price} | Proceeds: {proceeds:,.2f} | PnL: {profit_loss:,.2f} | Cash: {self.cash_remaining:,.2f}")
            sendeqtAlert(f"{self.client_name} - LIQUIDCASE EXIT: {shares} units | PnL: {profit_loss:,.0f} | Cash: {self.cash_remaining:,.0f}")
        else:
            logger.error(f"LIQUIDCASE SELL order rejected: {status_message}")
            sendeqtAlert(f"{self.client_name} - LIQUIDCASE SELL REJECTED: {status_message}")
    ...
    def analyze_current_portfolio(self):
        ...
        for stock in self.portfolio:
            ...
            self.portfolio[stock]["100_Days_EMA"] = 0 if stock == LIQUID_ETF else self.get_ema_for_date(stock, self.execution_date_str)
        ...
        logger.info("=" * 100)
        logger.info(f"Invested Capital: {invested_capital}, Current Holdings Value: {total_value_holdings}, Total Profit/Loss: {total_profit_loss}")
        logger.info("=" * 100)
        # Summary metrics
    ...
    def add_additional_amount_to_current_portfolio(self, additional_amount):
        ...
        logger.info(f"Investment per stock: {investment_per_stock:,.2f}")
        sendeqtAlert(f"{self.client_name} - Additional: {additional_amount:,.0f} | Per stock: {investment_per_stock:,.0f} | Holdings: {no_of_holdings}")

        current_stock_prices = self.get_last_price(list(self.portfolio.keys()), debug_mode=self.debug_mode)
        logger.info(f"{'Additional Amount':<20} {'Stock':<15} {'Price':<15} {'Spent':<15} {'Current':<15} {'Remaining Capital':<20}")
        for stock in self.portfolio:
            if stock == LIQUID_ETF:
                continue
            buy_price = current_stock_prices[stock]["last_price"]
            ...
            if shares > 0:
                ...
                if status == 'COMPLETE':
                    ...
                    self.portfolio[stock]["Current_Amount"] = round(average_price * self.portfolio[stock]["No_Of_Shares"], 2)
                    logger.info(f"{'':<20} {stock:<15} {buy_price:<15} {spent:<15} {self.portfolio[stock]['Current_Amount']:<15} {additional_amount:<20}")
                else:
                    logger.error(f"{'':<20} {stock:<15} Cannot be bought at this moment.. Order Rejected")
                    sendeqtAlert(f"{self.client_name} - BUY REJECTED (Additional): {stock} - {status_message}")
            else:
        ...
        sendeqtAlert(f"{self.client_name} - Additional capital done. Cash: {self.cash_remaining:,.0f}")
...
def perform_momentum_stock_options_on_date(execution_date_obj, userid, debug_mode):
    ...
    logger.info("=" * 80)
    logger.info(f"STARTING MOMENTUM STOCK OPTIONS EXECUTION: {client_name}")
    sendeqtAlert(f"{client_name} - Momentum script started for {execution_date_obj.strftime('%d-%m-%Y')}")
    ...
    # PHASE 1 - CHANGE 2: try/finally to always persist state even on crash
    try:
        ...
        if momentum.reset_and_fresh_rebalance:
            ...
            if momentum.additional_amount > 0 and momentum.portfolio:
                ...
                sendeqtAlert(f"{client_name} - Adding Rs.{momentum.additional_amount:,.0f} additional capital")
        ...
        if already_executed_flag:
            ...
            sendeqtAlert(f"{client_name} - Already executed on {momentum.execution_date_str}, skipping rebalance")
        if momentum.rebalance_counter - 1 == 0 and not already_executed_flag:
            ...
            if stocks_to_sell:
                ...
                sendeqtAlert(f"{client_name} - No stocks to sell at rebalance")
                old_stocks_deleted = {}
            ...
            if momentum.sip_amount > 0:
                ...
                sendeqtAlert(f"{client_name} - SIP {momentum.sip_amount:,.0f} added. Cash: {momentum.cash_remaining:,.0f}")

            new_stocks_added = {}
            if momentum.buy_stocks_or_not:
                # BULLISH — exit LIQUIDCASE first to free cash for momentum buys
                if LIQUID_ETF in momentum.portfolio:
                    liquid_exit = momentum.exit_liquid_etf()
                    write_exit_stocks_to_excel(liquid_exit, "LIQUIDCASE_EXIT")

                portfolio_count = sum(1 for s in momentum.portfolio if s != LIQUID_ETF)
                remaining_slots = TOP_N_INVEST - portfolio_count
                stocks_to_buy = momentum.get_stocks_to_buy(
                    remaining_slots,
                )
            ...
            # BEARISH — sweep remaining cash into LIQUIDCASE at the very end
            if not momentum.buy_stocks_or_not:
                momentum.sweep_cash_to_liquid_etf()

            # Resetting rebalance counter to 7
            momentum.rebalance_counter = momentum.momentum_settings.get(userid, {}).get("rebalance_counter", REBALANCE_COUNTER)
        else:
            if not already_executed_flag:
                momentum.rebalance_counter = momentum.rebalance_counter - 1
                sendeqtAlert(f"{client_name} - No rebalance today. Next rebalance in {momentum.rebalance_counter} day(s)")
                ...
                if remaining_slots > 0 and momentum.cash_remaining > 0:
                    ...
                    sendeqtAlert(f"{client_name} - {remaining_slots} empty slot(s), attempting buy. Cash: {momentum.cash_remaining:,.0f}")
                    if momentum.buy_stocks_or_not():
                        # BULLISH — exit full LIQUIDCASE first, then buy best available
                        if LIQUID_ETF in momentum.portfolio:
                            liquid_exit = momentum.exit_liquid_etf()
                            write_exit_stocks_to_excel(liquid_exit, "LIQUIDCASE_EXIT")
                        momentum.populate_stock_top_select_and_invest_data()
                        ema_stocks_to_buy = momentum.get_stocks_to_buy(remaining_slots)
                        momentum.buy_stocks_and_spend_capital(ema_stocks_to_buy)
                    else:
                        # BEARISH — sweep cash into LIQUIDCASE
                        if momentum.cash_remaining > 0:
                            momentum.sweep_cash_to_liquid_etf()
        ...
        # This ensures the DB reflects whatever broker operations actually completed.
        try:
            ...
            sendeqtAlert(
                f"{client_name} - DAY END {momentum.execution_date_str}\n"
            ...
            logger.info("=" * 80)
            logger.info("MOMENTUM STOCK OPTIONS EXECUTION COMPLETED SUCCESSFULLY")
...
def setup_logging(userid, date_str):
    ...
    log_path = os.path.join(userid, f"Logs_EMA")
    ...
    TOP_N_SELECT_FILE = os.path.join(
        log_path, rf"{cred_account_settings[userid]['client_name']}_{date_str}_Equity.txt")
